...
def main():
    ...
    tot_training_ite = len(trainloader) * config["train"]["epochs"]

    if config["train"]["optim"]["scheduler"]["class"].__name__ ==  "CyclicLR":
        cyc.cyclical_lr_determine_up_and_down_size(config, tot_training_ite)
        cyc.determine_base_lr(config)


    net_module = config["net_class"](NUM_CLASSES[config["data"]["dataset"]])

    make_subdirectory(config["train"]["checkpoint_path"])
    make_subdirectory(config["train"]["final_model_save_path"])

    config["train"]["pruning"]["scheduler"]["hyperparameters"]["tot_num_pruning_ite"] = tot_training_ite // config["train"]["pruning"]["hyperparameters"]["pruning_frequency"]
    ...
    if config["train"]["resume_training"]:
        if os.path.isfile(check_path:=config["train"]["checkpoint_path"]):
            ...
            epoch_start, ite_start = determine_epoch_and_ite_start(epoch_start, ite_start, config["train"]["epochs"], len(trainloader))
